...
#### 2. 변수 할당
    ...
    캐러셀 구현에 필요한 슬라이드와 버튼 요소들을 HTML 문서에서 가져와 변수에 할당합니다.

이 변수들은 이벤트 리스너를 추가하고, 슬라이드를 조작하는 데 사용됩니다.
...
#### 3. 변수 초기화
    ...
    현재 슬라이드 인덱스, 자동 재생 상태, 슬라이드 전환 간격 및 자동 재생 간격을 설정하는 변수를 선언하고 초기화합니다.
...
#### 4. 슬라이드 이동 및 인디케이터 업데이트

`moveSlide()`함수를 통해 인덱스에 따라 슬라이드를 이동시키고, ` updateIndicators()` 함수를 호출하여 현재 슬라이드에 해당하는 인디케이터를 업데이트합니다.
`slides.length` 를 사용하여 마지막 슬라이드에서 첫 번째 슬라이드로 원형 이동이 가능하도록 구현합니다.
...
#### 5. 자동 재생 기능
    ...
    슬라이드 자동 전환을 위한 startAutoPlay()와 자동 재생을 멈추기 위한 stopAutoPlay() 함수를 구현합니다. 이 함수들은 슬라이드가 일정 간격으로 자동으로 전환되도록 합니다.
...
사용자가 이전 또는 다음 버튼을 클릭할 때 슬라이드를 전환할 수 있도록 이벤트 리스너를 추가합니다. 버튼 클릭 시 자동 재생이 멈추고 슬라이드 이동 후 다시 자동 재생을 시작합니다.
...
인디케이터를 클릭하면 해당 슬라이드로 이동하도록 이벤트 리스너를 추가합니다. 인디케이터 클릭 시 자동 재생이 멈추고 다시 시작됩니다.
...
#### 8. 자동 재생 일시정지 및 재개
    ...
    플레이/일시 정지 버튼 클릭 시 자동 재생 상태를 전환할 수 있도록 이벤트 리스너를 추가합니다. 자동 재생이 재개될 때 버튼 텍스트가 변경됩니다.
...
#### 9. 슬라이드 속도 및 간격 변경 가능:

슬라이드 속도와 간격을 변경할 수 있는 입력 요소를 추가하고, 이를 통해 사용자가 직접 설정할 수 있도록 합니다. 설정이 변경되면 `moveSlide()`에서 css에 반영되어 슬라이드가 전환됩니다.
...
#### 10. 터치 스크린 장치에서 제스처 인식

터치 스크린 장치에서 사용자가 스와이프 제스처를 인식하여 슬라이드를 제어할 수 있도록 이벤트 리스너를 설정합니다. 사용자가 터치한 시작 위치와 종료 위치의 차이를 계산하여 스와이프 방향을 결정합니다.사용자가 터치한 시작 위치와 종료 위치의 차이를 계산하여 스와이프 방향을 결정합니다. 스와이프 제스처를 인식하기 위해, 사용자가 터치한 시작 위치를 startX 변수에 저장하고, 터치가 종료된 후 시작 위치와 종료 위치의 차이를 diffX로 계산합니다.
...
슬라이드를 초기화하고 자동 재생을 시작합니다. 초기화는 첫 번째 슬라이드를 현재 슬라이드로 설정하고, 자동 재생을 시작하는 역할을 합니다.
